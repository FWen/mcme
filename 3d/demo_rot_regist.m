...
for n=1:length(outlier_ratios)
    for k=1:50
        ...
        %% MCME
        [R] = mcme_rot_regist(pt1, pt2, sigma, q_ransac);
        angErr_mcme(k,n) = 2*asin(norm(G.T(1:3,1:3)-R,'fro')/(2*sqrt(2)));  
    end
end
...
%% rotation error
m=2;
x = [angErr_rs; angErr_mcme]/pi*180;
