...
for n=1:length(outlier_ratios)
    parfor k=1:50
        ...
        rng('shuffle')
        G = affine3d([randrot, zeros(3,1); 200*rand(1,3)-100, 1]); % rotation
        ...
        %% AM 
        [R, t] = sime_6dof_regist(pt1, pt2, sigma, q_ransac, t_ransac);
        angErr2(k,n) = 2*asin(norm(G.T(1:3,1:3)-R,'fro')/(2*sqrt(2)));
        trErr2(k,n)  = norm(G.T(1:3,1:3)*G.T(4,1:3)'-R*t);
        
        %% AM-R 
        [R, t] = sime_6dof_regist_sdr(pt1, pt2, sigma, q_ransac, t_ransac);
        angErr3(k,n) = 2*asin(norm(G.T(1:3,1:3)-R,'fro')/(2*sqrt(2)));
        trErr3(k,n)  = norm(G.T(1:3,1:3)*G.T(4,1:3)'-R*t);
    end
end

%% rotation error
m = 3;
x = [angErr1; angErr2; angErr3]/pi*180;
...
%% translation error
x = [trErr1; trErr2; trErr3];
g1 = kron([1:m]', ones(size(trErr1))); g1 = g1(:);
g2 = repmat(1:n,size(x,1),1); g2 = g2(:);
...
for k=1:size(trErr1,2)
    annotation(f,'textbox',[((k-1)+1.5)/(size(trErr1,2)+2), 0.075, 0.035, 0.075],...
    'String',{outlier_ratios(k)},'FitBoxToText','off', 'EdgeColor','none');
end
set(gca,'XTickLabel',{' '});
box_vars = findall(gca,'Tag','Box');
