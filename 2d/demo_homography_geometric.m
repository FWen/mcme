...
for k=1:length(imgs)
    ...
    %% AM
    tic
    [mmTheta, mmInliers] = sime_nonlinear(A0, b0, c0, d0, [], rsTheta_ref, th);
    ...
    %% AM-R
    tic
    [mmTheta, mmInliers] = sime_nonlinear_sdr(A0, b0, c0, d0, [], rsTheta_ref, th);
    mc(k,6) = length(mmInliers);
    rt(k,6) = toc + rsRuntime;
    H21 = inv(data.T2)*reshape([mmTheta; 1], [3,3])'*data.T1; % estimated homography
    scores(k,10) = compute_homo_score(H21, data.matches.X1, data.matches.X2);
end
...
p = semilogy(img_indx,scores(:,1),'r--+',img_indx,scores(:,2),'r--o',img_indx,scores(:,3),'g--+',...
    img_indx,scores(:,4),'g--o',img_indx,scores(:,5),'c--+',img_indx,scores(:,6),'c--o',...
    img_indx,scores(:,7),'k--+',img_indx,scores(:,8),'k--o',img_indx,scores(:,9),'b--^',img_indx,scores(:,10),'m--o','linewidth',1); 
xlabel('Image index'); grid on; ylabel('Score of estimated homography')
legend('RANSAC','RANSAC-r','L1','L1-r','IRW','IRW-r','EP','EP-r','AM','AM-R', 'Location', 'Best');

figure(2);
p = semilogy(img_indx,rt(:,1),'r--+',img_indx,rt(:,2),'g--*',img_indx,rt(:,3),'c--x',...
     img_indx,rt(:,4),'k--o',img_indx,rt(:,5),'b--^',img_indx,rt(:,6),'m--d','linewidth',1); 
xlabel('Image index'); grid on; ylabel('Runtime (second)');
legend('RANSAC','L1','IRW','EP','AM','AM-R','Location','Best');

figure(3);
semilogy(img_indx,mc(:,1),'r--+',img_indx,mc(:,2),'g--*',img_indx,mc(:,3),'c--x',...
         img_indx,mc(:,4),'k--o',img_indx,mc(:,5),'b--^',img_indx,mc(:,6),'m--d','linewidth',1); 
xlabel('Image index'); grid on; ylabel('Consensus size');
legend('RANSAC','L1','IRW','EP','AM','AM-R','Location','Best');
