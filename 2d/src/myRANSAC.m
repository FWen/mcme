...
function [M] = myRANSAC(x, fittingfn, distfn, degenfn, s, t, maxDataTrials, maxTrials)
    ...
    while (N > trialcount || trialcount < 1e3) % Force RANSAC to test at least 1e3 samples before it can quit
        
        % Select at random s datapoints to form a trial model, M.
        % Iave to check that they are not in
        % a degenerate configuration.n selecting these points we h
        degenerate = 1;
        count = 1;
        while degenerate
